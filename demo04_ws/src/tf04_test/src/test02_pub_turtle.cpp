...
void doPose(const turtlesim::Pose::ConstPtr& pose)
{
    ...
    // 关键点2：child_fram_id动态传入
    ts.child_frame_id = turtule_name;
    //坐标系偏移量
...
{    
    ...
    // 3.创建一个订阅对象,订阅/turtle1/pose
    // 关键点1：订阅的话题名词,/turtle1 /turtle2 动态传入
    ros::Subscriber sub = nh.subscribe(turtule_name + "/pose",100,doPose);
    // 4.回调函数处理订阅的消息:将位姿信息转换成坐标相对关系并发布(关注)
    // 5.spin()
    ros::spin();
    return 0;
